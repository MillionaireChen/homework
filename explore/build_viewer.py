...
:root {
  ...
  --b-refcopy:#7c3aed; --b-lead:#c2410c; --b-trunc:#dc2626; --b-refpre:#9333ea;
}
...
.card { background:var(--panel); border:1px solid var(--line); border-radius:12px;
  padding:20px 24px; margin-bottom:18px; max-width:960px; }
...
<script>
const DATA = __DATA__;
const FLAG_NAME = { ref_copy:"抄参考摘要", ref_prefix:"参考摘要截断版", lead_copy:"照抄原文开头", truncated:"疑似截断" };
...
n_tr = len(translations)
print(f"OK -> {out_path}  ({len(out)/1e6:.2f} MB, 翻译覆盖 {n_tr}/50 篇)")
