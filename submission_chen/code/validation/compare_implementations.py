...
def main() -> None:
    ...
    ax.set_xlabel("Claude score"); ax.set_ylabel("Codex score")
    ...
    ax.bar([i + 0.2 for i in x], gterm, 0.4, label="Codex", color="#f59e0b")
    ...
    # 4. complete-run routing outcomes
    c_soft = sum(1 for row in C.values() if not term(row))
    g_soft = sum(1 for row in G.values() if not term(row))
    c_terminal = len(C) - c_soft
    g_terminal = len(G) - g_soft
    fig, ax = plt.subplots(figsize=(6.4, 3.6))
    xpos = [0, 1]
    ax.bar(xpos, [c_soft, g_soft], width=0.58, label="Soft-scored", color="#2563eb")
    ax.bar(xpos, [c_terminal, g_terminal], width=0.58,
           bottom=[c_soft, g_soft], label="Terminal", color="#dc2626")
    for i, (soft_count, terminal_count) in enumerate(
        [(c_soft, c_terminal), (g_soft, g_terminal)]
    ):
        ax.text(i, soft_count / 2, str(soft_count), ha="center", va="center",
                color="white", fontweight="bold")
        ax.text(i, soft_count + terminal_count / 2, str(terminal_count),
                ha="center", va="center", color="white", fontweight="bold")
    ax.set_xticks(xpos); ax.set_xticklabels(["Claude", "Codex"])
    ax.set_ylabel("Candidates"); ax.set_ylim(0, 270)
    ax.set_title("Complete-run outcomes (250 candidates per implementation)", fontsize=11)
    ax.legend(frameon=False, ncol=2, loc="upper center")
    fig.tight_layout(); fig.savefig(args.outdir / "pipeline_outcomes_comparison.png", dpi=150); plt.close(fig)

    # 5. terminal-category comparison
    terminal_order = [
        "VERBATIM_SOURCE_COPY", "OFF_TOPIC", "FACTUAL_REVERSAL",
        "OBVIOUS_TRUNCATION", "FABRICATED_CONTENT",
    ]
    terminal_labels = ["Copy", "Off-topic", "Reversal", "Truncation", "Fabrication"]
    c_terminal_counts = Counter(term(row) for row in C.values() if term(row))
    g_terminal_counts = Counter(term(row) for row in G.values() if term(row))
    fig, ax = plt.subplots(figsize=(7.4, 3.6))
    x = list(range(len(terminal_order)))
    cvals = [c_terminal_counts.get(name, 0) for name in terminal_order]
    gvals = [g_terminal_counts.get(name, 0) for name in terminal_order]
    ax.bar([i - 0.19 for i in x], cvals, 0.38, label="Claude", color="#2563eb")
    ax.bar([i + 0.19 for i in x], gvals, 0.38, label="Codex", color="#f59e0b")
    for positions, values in [([i - 0.19 for i in x], cvals), ([i + 0.19 for i in x], gvals)]:
        for position, value in zip(positions, values):
            ax.text(position, value + 0.7, str(value), ha="center", fontsize=9)
    ax.set_xticks(x); ax.set_xticklabels(terminal_labels)
    ax.set_ylabel("Candidates"); ax.set_ylim(0, max(cvals + gvals) * 1.18)
    ax.set_title("Final terminal outcomes by category", fontsize=11)
    ax.legend(frameon=False)
    fig.tight_layout(); fig.savefig(args.outdir / "terminal_category_comparison.png", dpi=150); plt.close(fig)

    # 6. soft-label comparison
    label_order = ["EXCELLENT", "GOOD", "FINE", "MIXED", "POOR"]
    c_label_counts = Counter(row.get("quality_label") for row in C.values() if not term(row))
    g_label_counts = Counter(row.get("quality_label") for row in G.values() if not term(row))
    fig, ax = plt.subplots(figsize=(7.4, 3.6))
    x = list(range(len(label_order)))
    cvals = [c_label_counts.get(name, 0) for name in label_order]
    gvals = [g_label_counts.get(name, 0) for name in label_order]
    ax.bar([i - 0.19 for i in x], cvals, 0.38, label="Claude", color="#2563eb")
    ax.bar([i + 0.19 for i in x], gvals, 0.38, label="Codex", color="#f59e0b")
    for positions, values in [([i - 0.19 for i in x], cvals), ([i + 0.19 for i in x], gvals)]:
        for position, value in zip(positions, values):
            ax.text(position, value + 0.8, str(value), ha="center", fontsize=9)
    ax.set_xticks(x); ax.set_xticklabels([name.title() for name in label_order])
    ax.set_ylabel("Soft-scored candidates")
    ax.set_ylim(0, max(cvals + gvals) * 1.18)
    ax.set_title("Soft quality-label distribution", fontsize=11)
    ax.legend(frameon=False)
    fig.tight_layout(); fig.savefig(args.outdir / "soft_label_comparison.png", dpi=150); plt.close(fig)
