...
4. For survivors, optionally run `scripts/embedding_gate.py`. A candidate is compared only with its own assigned article; never with another article, whether or not the batch contains one. Similarity below the threshold is a weak cost-saving hint for the next step and is never terminal on its own.
