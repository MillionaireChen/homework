...
4. For survivors, run `scripts/embedding_gate.py`. **This stage is required**, and it requires a local Ollama server serving `qwen3-embedding:0.6b`. A run that cannot produce embedding evidence is an incomplete run, not a cheaper one: every survivor must carry a similarity value in its trace, and the grounding gate is entitled to know which candidates arrived as relevance suspects. A candidate is compared only with its own assigned article, never with another article, whether or not the batch contains one. Similarity below the threshold nominates the candidate for close reading at the next stage and is never terminal on its own. Required evidence, never a decision.
