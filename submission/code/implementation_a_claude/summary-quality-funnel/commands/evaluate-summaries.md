...
2. Optional hint: on survivors, run `scripts/embedding_gate.py` when a local Ollama server is available. It compares a candidate with its own assigned article only, and its similarity is never terminal.
