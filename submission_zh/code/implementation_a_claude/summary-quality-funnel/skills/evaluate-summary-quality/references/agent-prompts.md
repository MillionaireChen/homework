...
These are task briefs for the plugin's separate subagents: `summary-grounding-gate`, `summary-scorer`, `summary-reviewer`, and `summary-reporter`. Launch each in its own context. Local inference is permitted only for embedding relevance evidence.
