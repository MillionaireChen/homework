...
LABEL_ORDER = ["EXCELLENT", "GOOD", "FINE", "MIXED", "POOR"]
...
def reference_section(
    ...
    """Render the offline reference cross-check. Reference summaries are read at this
    stage only; every runtime stage is reference-free."""
    if not reference:
        return ""

    corpus = reference["corpus"]
    accuracy = reference["accuracy"]
    ordering = reference["within_article_ordering"]
    ...
    for name in CATEGORY_ORDER:
        ...
        if not entry:
            continue
        scores = entry["surviving_soft_scores"]
        matched = (
            "not labelled"
            if name == UNLABELLED
            else f"{entry['correct']}/{entry['count']}"
        )
        mean = number(scores["mean"]) if scores else "all scored 0"
        span = (
            f"{number(scores['minimum'])}–{number(scores['maximum'])}"
            if scores
            else "all scored 0"
        )
        lines.append(
            f"| {entry['description'].capitalize()} | {entry['count']} | {matched} | {mean} | {span} |"
        )
    table = header + "\n".join(lines)
    ...
    disagreements = [
        ...
        for name in CATEGORY_ORDER
        if name != UNLABELLED
        for item in categories.get(name, {}).get("disagreements", [])
    ]
    if disagreements:
        listed = ", ".join(
            f"`{item['summary_id']}` at {number(item['score'])}" for item in disagreements
        )
        count = len(disagreements)
        subject = "one candidate" if count == 1 else f"{count} candidates"
        verb = "was" if count == 1 else "were"
        disagreement_text = (
            f"The planted category expected a terminal outcome for {subject} that {verb} "
            f"instead kept in soft scoring: {listed}."
        )
    else:
        disagreement_text = "No labelled candidate received an unexpected outcome."

    chart_line = f"\n![Reference cross-check]({chart_relative_path})\n" if chart_relative_path else ""

    return f"""
## 4. Reference Cross-Check

Reference summaries were withheld from every runtime stage and are read only here. Labels come from the corpus alone: a candidate contained in its own article is a source copy, a candidate equal to its reference is a reference reproduction, a leading fragment of its reference is a truncation, and a candidate matching a different article is off-topic. {accuracy['gradable_records']} of {corpus['labelled_records']} records carry such a label; the remaining {corpus['unlabelled_records']} generated candidates carry none and are excluded from the accuracy figure.

{table}

The evaluator's outcome matched the planted category for {accuracy['correct']} of {accuracy['gradable_records']} labelled candidates ({number(float(accuracy['rate']) * 100)} percent). {disagreement_text} Across the {ordering['comparable_articles']} articles holding both a reference reproduction and a planted failure, {ordering['articles_with_planted_bad_at_or_above_reference']} articles placed a planted failure at or above the reference.

This is weak-label agreement, not validated accuracy. Reference reproductions are treated as acceptable summaries even though the source references are uneven, and the {corpus['unlabelled_records']} generated candidates that carry the fluent factual errors remain unlabelled.
{chart_line}"""
...
{reference_section(reference, reference_chart)}
## {5 if reference else 4}. Conclusion
...
def main() -> None:
    ...
    parser.add_argument(
        "--articles-input",
        type=Path,
        help="corpus articles JSONL; report stage only, enables the reference cross-check",
    )
    parser.add_argument(
        "--summaries-input",
        type=Path,
        help="corpus summaries JSONL; required together with --articles-input",
    )
    ...
    if bool(args.articles_input) != bool(args.summaries_input):
        raise SystemExit("--articles-input and --summaries-input must be supplied together")
    ...
    reference = None
    reference_chart_rel = None
    if args.articles_input:
        reference = build_reference_stats(
            load_rows(args.articles_input), load_rows(args.summaries_input), rows
        )
        reference_output = args.asset_dir / "reference_validation.json"
        reference_output.write_text(
            json.dumps(reference, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
        )
        reference_chart = plot_reference_validation(
            reference, args.asset_dir / "reference_validation.png"
        )
        reference_chart_rel = relative_markdown_path(reference_chart, args.report_output)
    ...
    if reference:
        print(f"reference validation: {args.asset_dir / 'reference_validation.json'}")
        print(f"chart: {args.asset_dir / 'reference_validation.png'}")
