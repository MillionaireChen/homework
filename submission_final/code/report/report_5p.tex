...
\hypersetup{pdftitle={[5-page] Judge as a Plugin: A Reference-Free Evaluation Funnel for Japanese News Summaries},
...
source of \texttt{scores.jsonl}); \textbf{B} $=$ Codex plugin (secondary).}
\newcommand{\abnote}{\tabnote{\textit{Note.} \abkey}}

\title{Judge as a Plugin: A Reference-Free Evaluation Funnel for Japanese News Summaries}
...
hosts. This report contributes the following.

\begin{enumerate}
\setlength{\itemsep}{1.5pt}

\item \emph{Data characterisation.} The 250 candidates form five
distinct populations rather than a quality gradient, only 141 of which the
corpus can label, and the references are not ground truth: 45 of 50 are
outscored by a generated candidate in their own article. The two dominant
failure modes, reversal and fabrication, are on topic by construction, so
no cheap signal can detect them.

\item \emph{A stated determinism--judgement boundary.} Hard constraints
terminate and the rest is graded on a soft rubric, and within the hard
constraints a script decides only what its measurement proves while a
Grounding Gate agent blind to the rubric screens grounding before any
scoring. The split settles 51 of 250 pairs deterministically and keeps 80
out of the most expensive stage.

\item \emph{Judge as a plugin.} Where LLM-as-a-judge is normally a
notebook and a prompt, we deliver the judge as an installable plugin
whose role contracts, rubric and few-shot bank are versioned files and
whose every record carries a replayable evaluation trace, so a third
party can run and audit the evaluator instead of reconstructing it. The
design is also portable, which lets us implement it twice and turn
delivery itself into a validation instrument (Section~III-F).

\item \emph{Validation by instruments that fail differently.} The two
implementations route all 250 pairs with zero arithmetic errors and zero
ordering inversions against the reference, agree with corpus-derived weak
labels at 0.915, and agree with each other on routing for 94.4\,\% of
candidates and on terminal category for 74 of 74. On the 109 candidates
carrying no derivable label, a third independent reading agrees with the
submitted run at $\rho = 0.960$ and places every terminated candidate at
the bottom of its distribution; every number quoted here is re-derived
from the shipped artefacts by one script.
...
implementation---\textbf{run~A} everywhere below---is a Claude Code
plugin: one slash command, one skill,
...
The same design is implemented a second time as a Codex plugin, and that
is \textbf{run~B} everywhere below. This is
...
Tables~\ref{tab:pop}--\ref{tab:third} give each check's numbers. The
prose says what each one can and cannot support.
...
\texttt{scores.jsonl}; \textbf{run~B is the Codex plugin}, an independent
second implementation of the same design on a different host, used as
cross-host validation and never as a fallback.

\subsection{Run-level results}
Table~\ref{tab:runs} is the whole outcome of both runs; A is the
submitted one. The two implementations land on the \emph{same} count for
the failure a script can prove---50 verbatim copies---and within one for
the failure an embedding can nominate, 16 against 17 off-topic; they
diverge only on the two categories a reading must raise, A stopping 13
reversals and 1 fabrication against B's 7 and 0. Their distributions
differ in shape, not centre: the means are 76.1 and 76.2, but B's
standard deviation is 19.0 against A's 15.1 and it awards 52
\textsc{excellent} against 28, so B is the more generous reader and A
stops eight more candidates. B's orchestration also sent 155 of 250
drafts back for a second round against A's 8, which is a fact about the
hosts rather than the design; both runs end with all 250 records approved
and nothing escalated. Every disagreement below follows from that
...
band: zero arithmetic and zero banding violations, the last two rows of
Table~\ref{tab:runs}. This is a floor, not a result---it says the
machinery did what it claims, nothing about whether the judgements are
right.
...
both implementations, 8/8 and 8/8. Run-to-run variance on repeated input
is therefore not the explanation for any spread reported below.

Fourteen further groups, 32 candidates in all, are byte-identical but
split across articles: the same text sits under its own article in one
place and under a foreign one in another. If the evaluator graded text
quality, both copies would score the same. They do not. In all 14
groups, in both implementations, the foreign placement was terminated at
0 as off-topic and the native placement was graded on its merits---the
text of \texttt{54083932\_9e7163bb} scores 87 under its own article and
0 as \texttt{44708203\_6d931562} under another. The evaluator conditions
on the pair, which is what the contract requires and what a text-only
metric cannot do.
...
Table~\ref{tab:pop} is the per-population outcome and
Fig.~\ref{fig:bands} the same data as distributions.
...
score columns cover survivors only. \emph{Agreement}: a candidate counts
as correct when its routing matches the population's expected
routing---copies, foreign placements and fragments should stop, reference
reproductions should not.}
\end{table}

Survivor ranges tell more than the means: fragments survive at 39--70 in
A and 34--63 in B, reference reproductions at 42--89 and 40--99,
generated candidates at 35--98 and 28--100, and no surviving fragment
reaches \textsc{good} in either run. Two things in Fig.~\ref{fig:bands}
matter more than the headline figure. The fragment population is graded
rather than uniformly stopped---a deliberate divergence from the label,
discussed in Section~IV-B---and in run~A \emph{no} reference reproduction
reached \textsc{excellent} while 28 generated candidates did, which is
the reference-quality problem of Section~I made quantitative: the
evaluator credits only what the article body supports. Run~B places 10
references in the top band and 42 generated candidates, so the ordering
survives the difference in temperament.
...
and the ordering is not degenerate: the median spread between best and
worst survivor is 26 points in A and 32 in B, every article retained two
to four survivors, and the reference reproduction sits at rank 2 in 43 of
50 articles in A and 36 of 50 in B, never below rank 3. It is
...
\quad 3 survivors          & 27 & 21 \\
\quad 4 survivors          & 19 & 26 \\
\quad 0, 1 or 5 survivors  & 0  & 0  \\
\addlinespace[3pt]
\multicolumn{3}{@{}l}{\textit{Score spread among survivors}}\\
\quad median               & 26 & 32 \\
\quad minimum / maximum    & 6 / 61 & 2 / 68 \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{}lrr@{}}
\toprule
 & \textbf{count} & \textbf{rate} \\
\midrule
\multicolumn{3}{@{}l}{\textit{Routing, $n=250$}}\\
Agreement (both terminal or both graded) & 236 & 0.944 \\
\quad both terminal                      & 74  & \\
\quad both graded                        & 162 & \\
\quad terminal in A only                 & 11  & \\
\quad terminal in B only                 & 3   & \\
...
Pearson $r$                              &     & 0.865 \\
Mean $|\Delta|$                          &     & 6.85 \\
Mean $\Delta$ (SD)                       &     & $-1.97$ (8.81) \\
\quad $|\Delta| \leq 5$                  & 86  & 0.531 \\
\quad $|\Delta| \leq 10$                 & 124 & 0.765 \\
\quad $|\Delta| \leq 15$                 & 145 & 0.895 \\
\quad $|\Delta| \leq 20$                 & 156 & 0.963 \\
Label identical                          & 88  & 0.543 \\
Label within one band                    & 157 & 0.969 \\
\addlinespace[3pt]
\multicolumn{3}{@{}l}{\textit{Per dimension, $n=162$: $\rho$ / mean $|\Delta|$}}\\
...
one-sided. All 11 pairs stopped only by A are severity calls on a defect
B also found---6 reversals, 4 fragments and 1 fabrication, which B scored
between 28 and 66---and the 3 stopped only by B were scored 58--71 by A.
Not one is a case where one implementation saw a defect the other missed
entirely: the two disagree about where the cliff is, not about which
candidates are near it, and all 14 come from the two populations where a
judgement call is required.

The per-dimension block needs a caveat. Faithfulness and coverage, which
carry 80 of the 100 points and do the discriminating, correlate at 0.864
and 0.882 with mean absolute differences of 4.25 of 50 and 2.96 of 30.
Coherence and conciseness correlate far worse, 0.635 and 0.243, but
differ by only 0.70 of 15 and 0.25 of 5 because both runs push almost
every survivor to the ceiling---range restriction, not disagreement, and
a sign that 15 and 5 points are more than these dimensions earn. Exact
agreement on the five-band label is only 88 of 162, the least flattering
number in the report; it is a banding artefact, since 96.9\,\% of pairs
fall within one band, but the bands are a convenience for a reader and
the ordering is what should be trusted.
...
same rubric but had no access to either run's output.
Table~\ref{tab:third} gives the result. On the 94 candidates all three
readings graded, the third read agrees with the primary run at
$\rho = 0.960$ and 4.09 points, and with the secondary run at
$\rho = 0.936$ and 5.95---in both cases more closely than the two
implementations agree with each other on the same 94.
...
reader never told they had been stopped.}
\end{table}

The lower block of Table~\ref{tab:third} is the part that matters. The
14 candidates the primary run terminated in this block were given a mean
of 32.5 by the independent read, range 20--53, against 77.0 for the
rest; the 8 the secondary run terminated came out at 34.2. An
independent reader who was never told those candidates had been stopped
placed every one of them at the bottom of the distribution, and the two
runs stopped candidates the third read scored at the same depth. The
terminal decisions on the unlabelled block are corroborated by a source
that did not make them.

This does not make the scores correct---three readings of the same rubric
can share a bias, and none is a human judgement---but it does establish
...
\texttt{data/}, the two run files and the third-read file. It prints 199
checks and exits non-zero if any of them moves, so a reader who distrusts
a number can recompute it in one command rather than take it on faith.
\section{Limitations}

\subsection{What the evaluation does not measure}
No human judgement enters anywhere. Every agreement number in
Section~III is agreement with a mechanically derived label or between
readings of the same rubric, and neither is accuracy, so the honest
ceiling is \emph{reproducible}, not \emph{correct}. The weak-label check
also treats reference reproductions as acceptable, which Section~I showed
is not safe: the worst scores 42, and there the label rather than the
evaluator is wrong. Beyond grammatical completeness the evaluation has no
view on style, register, audience fit or the newsworthiness of the facts
chosen---only on whether they are the article's main ones.

\subsection{What it measures poorly}
Truncation is the weak point, and the failure is structural. Candidate
\texttt{51395937\_e409196f} is the reference cut from 「…\mbox{方針を\mbox{発表}}した。」
to 「…\mbox{方針を\mbox{発表}}」; the Reviewer declined to call it truncated because
\mbox{発表} is a verbal noun discharging the clause, which is ordinary headline
register. The argument is sound and the conclusion wrong---only the
deleted した。 distinguishes \mbox{体言止め} from an amputated predicate, and a
reference-free evaluator does not have it. The fragment population
therefore splits: mid-token endings (\mbox{しかしそ}, \mbox{女性2人が車}) are visible
and missing them is a recall problem worth fixing, while \mbox{体言止め} endings
are undecidable under the contract, so the 5-of-17 figure overstates the
deficiency. The five-band labels are also less stable than the scores
(Section~III-F); bands are for readers, ordering for decisions. Cost is
uneven too: moving the truncation judgement from the script to the
Reviewer makes a fragment consume both before it is stopped, about six
percent of the scoring work.

\subsection{One structural gap, stated plainly}
The Reviewer in the submitted run held file-read tools while the corpus
...
guarantee by convention, and a convention is not evidence. A structurally
blind Reviewer is written and shipped in both plugins and no run has used
it; the right next step is to re-adjudicate the 85 terminal decisions
with it and publish the delta.

\subsection{What I would do next}
More data, above everything else. The report rests on 250 pairs from 50
articles of one publisher in one language, and every threshold here---the
0.50 similarity cut, the dimension maxima, the band boundaries---is
calibrated to that sample and should be assumed invalid outside it. The
immediate work is the same funnel over a larger Japanese corpus from
several publishers, then across languages, since hard constraints built
on Japanese sentence segmentation and \mbox{体言止め} register must be
re-derived rather than translated. Only then is it worth spending on
human adjudication of a stratified sample, controlled perturbations for
numbers, entities, negation and causality, and per-model re-calibration
