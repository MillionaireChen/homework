...
\hypersetup{pdftitle={[brief] Judge as a Plugin: A Reference-Free Evaluation Funnel for Japanese News Summaries},
...
\tikzset{
  stage/.style={rounded corners=1.6pt,draw=navy,line width=.35pt,fill=white,align=left,
                inner xsep=2.2mm,inner ysep=1.5mm,font=\sffamily\fontsize{5.1}{6.0}\selectfont},
...
The dataset is 50 articles of 483--3664 characters, each with a 41--164
character human reference and five opaque candidates of 22--302.
...
They are draws from distinct populations. A string-level scan of the corpus alone---no model, no
scoring---confirmed and counted them (Table~\ref{tab:census}): four are
mechanically derivable, the fifth is the remainder.
...
antecedent exists only in an image nobody is given. Any design scoring a candidate by its distance to the
reference rewards them.
...
\textbf{Survived.} Two 10-article probes: 0.707--0.868 matched,
0.163--0.398 mismatched. \\
\addlinespace[1pt]
...
delivery into a validation instrument (Section~III-F).
...
submitted run at $\rho = 0.960$. Every number quoted here is recomputable
in one command.
...
article. The review is not a rubber stamp---it rejected 2 of 53 physical
proposals, 8 of 37 grounding proposals (including all eight fabrication
claims) and 3 of 4 late scorer proposals, and escalated four truncations
the gate had not proposed. Disagreement surviving one revision round is
preserved as low confidence rather than averaged away.

\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=0mm]
\node[det,text width=52mm] (s1) {%
...
\node[rev,below=1.4mm of s1] (r1) {Reviewer · 51 confirmed, 2 returned};
\node[emb,below=1.4mm of r1,text width=52mm] (s2) {%
  \textbf{Embedding relevance} · \texttt{qwen3-embedding:0.6b}\\[.4mm]
  own article only · required · never terminal · 19 below 0.50};
\node[sem,below=1.4mm of s2,text width=52mm] (s3) {%
...
\node[rev,below=1.4mm of s3] (r2) {Reviewer · 29 confirmed, 8 returned};
\node[scr,below=1.4mm of r2,text width=52mm] (s4) {%
  \textbf{Anchor + claim-grounded scoring}\\[.4mm]
  50 cached anchors · atomic claims · 4 dimensions\\
  semantic backstop: 4 late proposals};
\node[rev,below=1.4mm of s4] (r3) {Reviewer · 157 approve · 5 revise · 4 escalate};
\node[scr,below=1.4mm of r3,text width=52mm,align=center] (out) {%
...
Score against the
reference &
Unavailable in production, and wrong here: generated candidates outscored
45 of 50 references. \\
\addlinespace[1pt]
...
which is the instrument for the cross-host check in Section~III-F. Both
...
dimensions and every label falls inside its band---the last two rows of
Table~\ref{tab:runs}. This is a floor, not a result.

\subsection{Determinism, and a natural experiment in pair conditioning}
The corpus supplies two free instruments (Section~I-A).
...
references in the top band against 42 generated candidates, so the
ordering survives the difference in temperament.
...
\begin{axis}[
  ...
  legend style={font=\sffamily\fontsize{5.6}{6.4}\selectfont,
                at={(0.5,1.42)},anchor=north,legend columns=6,
...
on the five-band label is only 88 of 162, the least flattering number in
the report; it is a banding artefact, since 96.9\,\% of pairs fall within
one band, but the bands are a convenience for a reader and the ordering is
...
\begin{axis}[
  ...
  label style={font=\sffamily\fontsize{5.1}{6.0}\selectfont},
...
\end{axis}
\begin{axis}[
  at={(sc.below south west)}, anchor=north west, yshift=-4mm,
  width=\columnwidth, height=22mm,
...
$\rho = 0.936$, both more closely than the two implementations agree with
each other.
...
judgement, but the signal there is reproducible rather than
idiosyncratic.

\subsection{Reproducing the numbers in this report}
Every quantity quoted above is re-derived by a checker reading only the
corpus, the two run outputs and the third-read file. It prints 199 checks
and exits non-zero if any moves, so a doubted number can be recomputed in
one command.
\section{Limitations}

\subsection{What the evaluation does not measure}
No human judgement enters anywhere. Every agreement number in Section~III
is agreement with a mechanical label or between readings of the same
rubric, and neither is accuracy: the honest ceiling is
\emph{reproducible}, not \emph{correct}. The weak-label check also treats
reference reproductions as acceptable, which is not safe---the worst scores
42, and there the label is wrong, not the evaluator. Beyond grammatical completeness it has no view on style,
register, audience fit or newsworthiness---only on whether the facts
chosen are the article's main ones.

\subsection{What it measures poorly}
Truncation is the weak point, and structurally so. One candidate is a
reference cut from 「…\mbox{方針を\mbox{発表}}した。」 to 「…\mbox{方針を\mbox{発表}}」;
the Reviewer declined to call it truncated, since \mbox{発表} is a verbal
noun discharging the clause, ordinary headline register. The argument is sound and the conclusion wrong---only the deleted した。
distinguishes \mbox{体言止め} from an amputated predicate, and a
reference-free evaluator does not have it. The fragment population
therefore splits: mid-token endings (\mbox{しかしそ}, \mbox{女性2人が車}) are
visible and missing them is a recall problem worth fixing, while
\mbox{体言止め} endings are undecidable under the contract, so the 5-of-17
figure overstates the deficiency. The five-band labels are also
less stable than the scores (Section~III-F), and cost is uneven: moving
the truncation judgement to the Reviewer makes a fragment consume both
stages before it is stopped, about six percent of the scoring work.

\subsection{One structural gap, stated plainly}
The Reviewer in the submitted run held file-read tools while the corpus
sat on disk with its reference column intact. Its inputs carried no
reference field and the artifacts show none was used, but that is a
convention, and a convention is not evidence. A structurally blind
Reviewer is shipped in both plugins but unused; the next step is to
re-adjudicate the 85 terminal decisions with it and publish the delta,
which is the honest measure of how much a reachable reference mattered.
...
0.50 similarity cut, the dimension maxima, the band boundaries---should be
assumed invalid outside that sample. The immediate work is the same funnel
over a larger Japanese corpus from several publishers, then across languages, since constraints built on Japanese sentence
segmentation and \mbox{体言止め} register must be re-derived rather than
translated. Only then is human adjudication of a stratified sample, and
