...
every number here is recomputable.
...
references in the top band against 42 generated candidates, so the ordering
survives.
...
$\rho = 0.936$, both closer than the two implementations agree. Every quantity quoted in
this report is re-derived by a checker that reads only the corpus and the
run outputs, and that fails if any of its 199 assertions moves.
...
Truncation is the weak point, and structurally so: a reference cut from
...
worth fixing, while \mbox{体言止め} endings are undecidable under a
reference-free contract, so the 5-of-17 figure overstates the deficiency.
...
that can settle it, and no stage stops a candidate on its own authority.
On this corpus it held---no ordering inversion against a reference, 0.915
against the weak labels, 94.4\,\% routing agreement with an independent
twin, and $\rho = 0.960$ from a third reading. And because it installs as
a plugin with every score carrying its trace, the next corpus is a run,
not a rebuild.
