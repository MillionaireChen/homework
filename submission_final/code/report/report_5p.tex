...
antecedent exists only in an image nobody is given. Any design scoring a candidate by its distance to the reference rewards
them.
...
carrying no derivable label a third independent reading agrees with the submitted run at $\rho = 0.960$, and
every number here is recomputable in one command.
...
reader who cannot check the source; copying and truncation rank above them, a copy at least containing true
sentences.
...
ordering survives the difference.
...
$\rho = 0.936$, both closer than the two implementations agree with each
other.
