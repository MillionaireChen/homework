...
\begin{table}[t]
\caption{Run-level results. A is the primary Claude-plugin run and the
source of \texttt{scores.jsonl}; B is the independent Codex-plugin run.}
\label{tab:runs}
\centering\footnotesize
\begin{tabular}{@{}lrr@{}}
\toprule
...
\begin{table}[t]
\caption{Outcome by corpus-derived population. ``Stop'' is the count
...
\begin{table}[t]
\caption{Cross-implementation agreement. Routing over all 250 pairs;
score statistics over the 162 pairs both implementations graded.
$\Delta = \text{A} - \text{B}$.}
\label{tab:xhost}
\centering\footnotesize
\begin{tabular}{@{}lrr@{}}
...
\begin{table}[t]
\caption{Third independent reading of the 109 candidates that carry no
derivable label. The upper block is the 94 that all three readings
graded; the lower block is the candidates a run terminated, scored by a
reader who was never told they had been stopped.}
\label{tab:third}
\centering\footnotesize
\begin{tabular}{@{}lrrrr@{}}
\toprule
